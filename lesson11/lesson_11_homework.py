...
# with open('lesson11/Phonebook_homework.json', 'w') as book: #Одноразовий запис телефонного довідника в файл json, для тестування програми. В подальшому цю команду і попередній словник закоментуватиб щоб не перезаписувати файл.
#     json.dump(phonebook, book, indent=2)


def open_phonebook(): # Функція завантаження телефонного довідника
    with open('lesson11/Phonebook_homework.json', 'r') as book1:
        result = json.load(book1)
        return result

def write_phonebook(y): # Функція перезапису довідника
    with open('lesson11/Phonebook_homework.json', 'w') as book2:
        json.dump(y, book2, indent=2)
...
def find_first_name(): # Шукаємо контакт за ім'ям
    ...
    if len(result) >= 1:
        ...
        return f"Контактів з ім'ям {first_name} не знайдено."
